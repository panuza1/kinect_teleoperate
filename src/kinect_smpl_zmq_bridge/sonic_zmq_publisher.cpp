...
SonicZmqPublisher::SonicZmqPublisher(int port, const std::string& bind_host) : context_(static_cast<_zctx_t*>(zmq_ctx_new())), socket_(nullptr) {
    ...
    if (!socket_) throw std::runtime_error("zmq_socket failed");
    const std::string endpoint = "tcp://" + bind_host + ":" + std::to_string(port);
    if (zmq_bind(socket_, endpoint.c_str()) != 0) throw std::runtime_error(zmq_strerror(zmq_errno()));
}
...
void SonicZmqPublisher::publish_planner(const SonicPoseFrame& frame) {
    const float vx = frame.velocity_mps[0];
    const float vy = frame.velocity_mps[1];
    const float speed = std::sqrt(vx * vx + vy * vy);
    if (!std::isfinite(speed) || !std::isfinite(frame.yaw_rate_rps)) return;
    const bool moving = speed > 0.05f || std::fabs(frame.yaw_rate_rps) > 0.05f;
    ...
    std::array<float, 3> facing{std::cos(yaw), std::sin(yaw), 0.0f};
