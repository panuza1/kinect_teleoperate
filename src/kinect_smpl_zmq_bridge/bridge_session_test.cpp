...
namespace {
void set(k4abt_skeleton_t& s, k4abt_joint_id_t id, float x, float y, float z = 1000) {
    auto& j = s.joints[id];
...
k4abt_skeleton_t neutral() {
    ...
    set(s, K4ABT_JOINT_SHOULDER_LEFT, -250, -400);
    set(s, K4ABT_JOINT_SHOULDER_RIGHT, 250, -400);
    set(s, K4ABT_JOINT_WRIST_LEFT, -300, 0);
    set(s, K4ABT_JOINT_WRIST_RIGHT, 300, 0);
    set(s, K4ABT_JOINT_FOOT_LEFT, -100, 900);
    set(s, K4ABT_JOINT_FOOT_RIGHT, 100, 900);
    set(s, K4ABT_JOINT_ANKLE_LEFT, -100, 790);
    set(s, K4ABT_JOINT_ANKLE_RIGHT, 100, 790);
    return s;
}
...
int main(int argc, char** argv) {
    ...
    sample.skeleton.joints[K4ABT_JOINT_WRIST_LEFT].position.xyz.y -= 250;
    ...
    sample.skeleton.joints[K4ABT_JOINT_PELVIS].confidence_level = K4ABT_JOINT_CONFIDENCE_NONE;
    result = step();
    assert(result.state == BridgeState::LOW_CONFIDENCE && result.publish && !result.planner);
    sample.skeleton.joints[K4ABT_JOINT_PELVIS].confidence_level = K4ABT_JOINT_CONFIDENCE_HIGH;
          ...
          ("kinect_bridge_session_" + std::to_string(getpid()) + ".trace");
    {
        ...
        for (int i = 0; i < trace_frames; ++i) {
            ...
            if (argc > 1) {
                ...
                sample.skeleton.joints[K4ABT_JOINT_WRIST_LEFT].position.xyz.y -= 300 * (arm(10) + arm(30));
                sample.skeleton.joints[K4ABT_JOINT_WRIST_RIGHT].position.xyz.y -= 300 * (arm(20) + arm(30));
            ...
            } else if (i == 70) sample.skeleton.joints[K4ABT_JOINT_WRIST_RIGHT].position.xyz.y -= 200;
